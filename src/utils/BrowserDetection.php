...
class BrowserDetection {
	 ...
	 *        	(optional) The user agent to work with. Leave empty for the current user agent
	...
	public function __toString() {
		...
		$values[] = array(
			'label' => 'Is AOL optimized',
			'value' => $this->isAol() ? 'true' : 'false'
		);
		$values[] = array(
			'label' => 'AOL version',
	 ...
	 *
	 * @param string $sourceVer
	 *        	The source version number.
	 * @param string $compareVer
	...
	public function compareVersions($sourceVer, $compareVer) {
		...
		foreach ($sourceVer as $i => $srcVerPart) {
			if ($srcVerPart > $compareVer[$i]) {
				...
				if ($srcVerPart < $compareVer[$i]) {return - 1;}
			}
		}
	...
	/**
	 * Get the version of AOL (if any).
	 * AOL releases "optimized" Internet Explorer and Firefox versions. In the making
	 ...
	 * All of the return values are class constants. You can compare them like this:
	...
	/**
	 * Get the name and version of the browser emulated in the compatibility view mode (if any).
	 * Since Internet
	...
	/**
	 * Get the name of the platform family on which the browser is run on (such as Windows, Apple, iPhone, etc.).
	 * All of
	...
	/**
	 * Get the platform version on which the browser is run on.
	 * It can be returned as a string number like 'NT 6.3' or
	 ...
	 *
	 * @param boolean $returnVersionNumbers
	 *        	(optional) Determines if the return value must be versions numbers as a
	 *        	string (true) or the version name (false).
	 * @param boolean $returnServerFlavor
	 *        	(optional) Since some Windows NT versions have the same values, this flag
	...
	public function getPlatformVersion($returnVersionNumbers = false, $returnServerFlavor = false) {
		if ($this->_platformVersion == self::PLATFORM_VERSION_UNKNOWN || $this->_platformVersion == '') {return self::PLATFORM_VERSION_UNKNOWN;}
	...
	/**
	 * Determine if the browser is executed from a 64-bit platform.
	 * Keep in mind that not all platforms/browsers report
	...
	/**
	 * Determine if the browser is from AOL.
	 * AOL releases "optimized" Internet Explorer and Firefox versions. In the
	...
	/**
	 * Determine if the browser is in compatibility view or not.
	 * Since Internet Explorer 8, IE can be put in
	 ...
	 *        	(optional) The value of the user agent. If an empty string is sent (default),
	...
	public function getClientIP() {
		...
		if (isset($_SERVER['HTTP_CLIENT_IP']) && array_key_exists('HTTP_CLIENT_IP', $_SERVER)) {
			...
			$ip = $_SERVER['REMOTE_ADDR'] ? '0.0.0.0' : null;
		}
	...
	/**
	 * Convert the Android version numbers to the operating system name.
	 * For instance '1.6' returns 'Donut'.
	 *
	 * @access protected
	 * @param string $androidVer
	...
	protected function checkBrowserBlackBerry() {
		...
		// Tablet OS check
		if ($this->checkSimpleBrowserUA('RIM Tablet OS', $this->_agent, self::BROWSER_TABLET_OS, true)) {return true;}
		
		// Version 6, 7 & 10 check (versions 8 & 9 does not exists)
		if ($this->checkBrowserUAWithVersion(array(
			'BlackBerry',
			'BB10'
	 ...
	 * Firebird was the name of Firefox from version 0.6 to 0.7.1.
	 *
	 * @access protected
	...
	protected function checkBrowserFirefox() {
		...
		if (preg_match('/.*Firefox[ (\/]*([a-z0-9.-]*)/i', $this->_agent, $matches) && stripos($this->_agent, 'Safari') === false) {
	 ...
	 * The browser was discontinued on September 27, 2008.
	 *
	 * @access protected
	...
	protected function checkBrowserInternetExplorer() {
		...
		if ($this->checkSimpleBrowserUA(array(
			'IEMobile',
			'MSPIE'
		), $this->_agent, self::BROWSER_IE_MOBILE, true)) {return true;}
		...
		if ($this->containString($this->_agent, array(
			...
			if (stripos($this->_agent, 'Trident') !== false) {
				...
				if (stripos($this->_agent, 'rv:') !== false) {
					if ($this->checkSimpleBrowserUA('Trident', $this->_agent, self::BROWSER_IE, false, false, 'rv:')) {return true;}
				} else {
	...
	/**
	 * Determine if the browser is the MSNBot crawler or not.
	 * In October 2010 it was replaced by the Bingbot robot.
	 *
	 * @access protected
	 ...
	 * The browser is discontinued since November 2001.
	 *
	 * @access protected
	...
	/**
	 * Determine if the browser is Netscape or not.
	 * Official support for this browser ended on March 1st, 2008.
	 *
	 * @access protected
	...
	protected function checkBrowserNetscape() {
		...
		if ($this->checkSimpleBrowserUA(array(
			...
			'Navigator',
			'Netscape6'
		), $this->_agent, self::BROWSER_NETSCAPE)) {return true;}
	...
	protected function checkBrowserNokia() {
		...
		if ($this->checkSimpleBrowserUA(array(
			...
			'S60',
			'S40OviBrowser'
		), $this->_agent, self::BROWSER_NOKIA, true)) {return true;}
	...
	protected function checkBrowserOpera() {
		if ($this->checkBrowserUAWithVersion('Opera Mobi', $this->_agent, self::BROWSER_OPERA_MOBILE, true)) {return true;}
		
		if ($this->checkSimpleBrowserUA('Opera Mini', $this->_agent, self::BROWSER_OPERA_MINI, true)) {return true;}
		...
		if (! $found || $version == '') {
			if ($this->checkSimpleBrowserUA('Opera', $this->_agent, self::BROWSER_OPERA)) {return true;}
		}
		
		if (! $found && $this->checkSimpleBrowserUA('Chrome', $this->_agent, self::BROWSER_CHROME)) {
			if ($this->checkSimpleBrowserUA('OPR/', $this->_agent, self::BROWSER_OPERA)) {return true;}
		}
	 ...
	 * Phoenix was the name of Firefox from version 0.1 to 0.5.
	 *
	 * @access protected
...
$this->checkBrowserMsnTv() ||            /* MSN TV is based on IE so we must check for MSN TV before IE */
        $this->checkBrowserInternetExplorer() || $this->checkBrowserOpera() ||            /* Opera must be checked before Firefox, Netscape and Chrome to avoid conflicts */
        ...
        $this->checkBrowserIceWeasel() || $this->checkBrowserGaleon() ||           /* Galeon is based on Firefox and needs to be tested before Firefox is tested */
        $this->checkBrowserFirefox() ||
        /* Current browsers that don't need to be detected in any special order */
        $this->checkBrowserKonqueror() || $this->checkBrowserLynx() || $this->checkBrowserAmaya() ||
        /* Mobile */
        $this->checkBrowserAndroid() || $this->checkBrowserBlackBerry() || $this->checkBrowserNokia() ||
        /* Bots */
        $this->checkBrowserGooglebot() || $this->checkBrowserBingbot() || $this->checkBrowserMsnBot() || $this->checkBrowserSlurp() || $this->checkBrowserYahooMultimedia() || $this->checkBrowserW3CValidator() ||
        /* WebKit base check (after most other checks) */
        $this->checkBrowserSafari() ||
        /* Deprecated browsers that don't need to be detected in any special order */
        $this->checkBrowserFirebird() || $this->checkBrowserPhoenix() ||
	...
	protected function checkBrowserSafari() {
		...
		$found = $this->checkBrowserUAWithVersion(array(
			'Safari',
			'AppleWebKit'
	...
	/**
	 * Test the user agent for a specific browser that use a "Version" string (like Safari and Opera).
	 * The user agent
	 * should look like: "Version/1.0 Browser name/123.456" or "Browser name/123.456 Version/1.0".
	 *
	 * @access protected
	 * @param mixed $uaNameToLookFor
	 *        	The string (or array of strings) representing the browser name to find in the user
	 *        	agent.
	 * @param string $userAgent
	 ...
	 *        	(optional) Determines if the browser is a robot or not.
	...
	protected function checkPlatform() {
		...
		if ($this->containString($this->_agent, array(
			...
			$this->setMobile(true);
		} else if ($this->containString($this->_agent, array(
			'BlackBerry',
			'BB10',
			'RIM Tablet OS'
		))) {
	 ...
	 *        	(optional) The separator string used to split the browser name and the version number in
	 *        	the user agent.
	 * @return boolean Returns true if we found the browser we were looking for, false otherwise.
	 */
	 ...
	 * @param mixed $needle
	 *        	The string to search for. Can be a string or an array of strings if multiples values are to
	 *        	be searched.
	 * @param boolean $insensitive
	 *        	(optional) Determines if we do a case-sensitive search (false) or a case-insensitive one
	 *        	(true).
	...
	/**
	 * Convert the macOS version numbers to the operating system name.
	 * For instance '10.7' returns 'Mac OS X Lion'.
	 *
	 * @access protected
	 * @param string $macVer
	 ...
	 * @access protected
	 * @param boolean $isAol
	 *        	Value that tells if the browser is AOL or not.
	 */
	 ...
	 * @access protected
	 * @param string $version
	 *        	The version of AOL (will be cleaned).
	 */
	...
	/**
	 * Convert the Windows NT family version numbers to the operating system name.
	 * For instance '5.1' returns
	 * 'Windows XP'.
	 *
	 * @access protected
	 * @param string $winVer
	...
	protected function windowsNTVerToStr($winVer, $returnServerFlavor = false) {
		...
		if ($this->compareVersions($cleanWinVer, '11') >= 0) {
			...
			} else if ($this->compareVersions($cleanWinVer, '5') < 0 && $this->compareVersions($cleanWinVer, '3') >= 0) {return self::PLATFORM_WINDOWS . ' NT ' . $winVer;}
		}
	...
	/**
	 * Convert the Windows 3.x & 9x family version numbers to the operating system name.
	 * For instance '4.10.1998'
	 * returns 'Windows 98'.
	 *
	 * @access protected
	 * @param string $winVer
